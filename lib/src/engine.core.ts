...
export const Coalesce: IEngine = {
  Physics() {
    ...
    let resolveCollision = function(s1, s2, collisionInfo) {
      ...
      let jT =
        -(1 + newRestituion) * relativeVelocity.dot(tangent) * newFriction;
      jT =
        jT /
        (s1.mInvMass +
          s2.mInvMass +
        ...
        for (i = 0; i < this.Core.mAllObjects.length; i++) {
          for (j = i + 1; j < this.Core.mAllObjects.length; j++) {
            if (this.Core.mAllObjects[i].boundTest(this.Core.mAllObjects[j])) {
              if (
                this.Core.mAllObjects[i].collisionTest(
                  this.Core.mAllObjects[j],
                  collisionInfo
                )
              ) {
                ...
                if (
                  collisionInfo
                    .getNormal()
                    .dot(
                      this.Core.mAllObjects[j].mCenter.subtract(
                        this.Core.mAllObjects[i].mCenter
                      )
                    ) < 0
                ) {
